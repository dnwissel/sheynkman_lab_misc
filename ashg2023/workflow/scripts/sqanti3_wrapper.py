...
def write_junctionInfo(
    ...
    for junction_index, (d, a) in enumerate(trec.junctions):
        ...
        qj = {
            ...
            if min_diff_s == min_diff_s
            else "NA",  # check if min_diff is actually nan
            "diff_to_Ref_end_site": min_diff_e
            if min_diff_e == min_diff_e
            else "NA",  # check if min_diff is actually nan
            "bite_junction": "TRUE"
            if (
            ...
            "sample_with_cov": sum(
                [cov_uniq > 0 for (cov_uniq, cov_multi) in sample_cov.values()]
            )
            if covInf is not None
            else "NA",
            "total_coverage_unique": sum(
                [cov_uniq for (cov_uniq, cov_multi) in sample_cov.values()]
            )
            if covInf is not None
            else "NA",
            "total_coverage_multi": sum(
                [cov_multi for (cov_uniq, cov_multi) in sample_cov.values()]
            )
            if covInf is not None
            else "NA",
        }
